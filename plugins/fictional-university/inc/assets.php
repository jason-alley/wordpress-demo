...
define( 'FICTIONAL_UNIVERSITY_ASSET_MAP', read_asset_map( dirname( __DIR__ ) . '/build/assetMap.json' ) );
define( 'FICTIONAL_UNIVERSITY_ASSET_MODE', FICTIONAL_UNIVERSITY_ASSET_MAP['mode'] ?? 'production' );
...
function get_asset_hash( string $asset ) : string {
	return get_asset_property( $asset, 'hash' )
		?? FICTIONAL_UNIVERSITY_ASSET_MAP['hash']
		?? '1.0.0';
}
...
function get_asset_property( string $asset, string $prop ) : ?string {
	...
	$asset_property = FICTIONAL_UNIVERSITY_ASSET_MAP[ $entrypoint ][ $type ][ $prop ] ?? null;
...
function inline_locale_data( string $to_handle ) {
	...
	wp_add_inline_script(
		$to_handle,
		'wp.i18n.setLocaleData( ' . wp_json_encode( $locale_data ) . ", 'fictional-university' );"
	);
}
